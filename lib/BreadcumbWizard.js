...
class BreadcumbWizard extends React.Component{
 constructor(props){
 	super(props)
 	this.state= {
 		arr: []
 ...
 componentDidMount(){
 	 let currentData = this.props.jsonData.data.filter((item) => item.path ===this.props.routePath )[0]
   let ds = currentData && currentData.parent_id  ? this.renderElement(currentData,this.props.jsonData) : null
 }

 componentWillReceiveProps(nextProps){
 		this.setState({arr: []})
	 	if(nextProps !== this.props ){
	 		let currentData = this.props.jsonData.data.filter((item) => item.path ===nextProps.routePath )[0]
	    let ds = currentData && currentData.parent_id  ? this.renderElement(currentData,this.props.jsonData) : null
	 	}
 }
...
renderElement(currentData, jsonData){
         ...
         return(
         	<React.Fragment key={item.name}>
	         	...
	         	<li className="nav-item">
	         	  <a href="javascript:void(0)" onClick={() => this.redirectData(item.path)} >{item.name}</a>
						</li>
 ...
 render(){
  ...
  let currentData = data.filter((item) => item.path ===this.props.routePath )[0]
...
export default BreadcumbWizard;